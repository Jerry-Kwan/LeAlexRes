...
parser.add_argument('--size', type=int, default=5, help='LRN size')
parser.add_argument('--alpha', type=float, default=0.0001, help='LRN alpha')
parser.add_argument('--beta', type=float, default=0.75, help='LRN beta')
parser.add_argument('--k', type=int, default=2, help='LRN k')
...
def save_checkpoint(state, is_best, args, filename='checkpoint_m1.pth.tar'):
    path = os.path.join(args.save_home, filename)
    torch.save(state, path)
    if is_best:
        shutil.copyfile(path, os.path.join(args.save_home, 'model_best_m1.pth.tar'))

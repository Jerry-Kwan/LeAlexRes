...
def main():
    ...
    # create model
    model = mm.MyResNet18(num_classes=args.num_classes)
...
def save_checkpoint(state, is_best, args, filename='checkpoint_ori.pth.tar'):
    path = os.path.join(args.save_home, filename)
    torch.save(state, path)
    if is_best:
        shutil.copyfile(path, os.path.join(args.save_home, 'model_best_ori.pth.tar'))
